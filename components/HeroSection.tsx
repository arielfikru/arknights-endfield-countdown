...
export const HeroSection: React.FC<HeroSectionProps> = ({ phase, accentColor, targetDate, label }) => {
  ...
  return (
    <section className="relative min-h-screen w-full flex flex-col justify-center overflow-hidden pt-10 pb-10">
      ...
      <div className="absolute inset-0 z-0">
        <img
          src={`${import.meta.env.BASE_URL}${isAnniversary ? 'kv-2.9326351a.jpg' : '014c0e72b163ae2a85af0b49f98ae805.jpg'}`}
          alt=""
      ...
      <div className="relative z-10 container mx-auto px-6 md:px-12 h-full flex flex-col md:flex-row items-center md:items-end gap-12 mt-12 md:mt-0">
        ...
        <div className="flex-1 w-full text-center md:text-left space-y-6">
          <div className="flex items-center justify-center md:justify-start gap-2 font-mono-tech text-sm tracking-[0.2em] mb-2 animate-pulse" style={{ color: accentColor }}>
            <span>&gt;&gt;</span>
            <span>{isAnniversary ? 'CELEBRATION_ACTIVE: ARKNIGHTS ENDFIELD' : 'TARGET_LOCK: ARKNIGHTS ENDFIELD'}</span>
          </div>
          ...
          <p className="text-gray-400 max-w-lg mx-auto md:mx-0 font-mono-tech text-sm md:text-base border-l-2 pl-4 animate-fade-in-up delay-300" style={{ borderColor: accentColor }}>
            {isAnniversary
              ? 'Celebrating one year of strategic exploration on Talos-II. The journey continues with new challenges.'
              : 'Real-time 3D RPG with strategic elements. Explore the planet Talos-II.'
            }
          </p>
          ...
          <div className="flex flex-col md:flex-row items-center gap-6 pt-8 animate-fade-in-up delay-500">
            <a
              ...
              className="relative text-black px-8 py-4 font-bold text-lg uppercase tracking-wider clip-button hover:bg-white transition-all hover:scale-105 active:scale-95 flex items-center gap-2 group overflow-hidden"
              style={{ backgroundColor: accentColor }}
            >
              <span className="relative z-10 flex items-center gap-2">
                {isAnniversary ? 'Join Celebration' : 'Pre-Register'} <ChevronRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </span>
        ...
        <div className="flex-1 w-full flex justify-center md:justify-end animate-fade-in-up delay-700">
          <div className="bg-black/80 backdrop-blur-md border-t-2 p-6 md:p-10 w-full max-w-xl relative animate-pulse-glow" style={{ borderColor: accentColor }}>
            {/* Decorative corners */}
            <div className="absolute top-0 right-0 w-4 h-4 border-t-2 border-r-2" style={{ borderColor: accentColor }}></div>
            <div className="absolute bottom-0 left-0 w-4 h-4 border-b-2 border-l-2" style={{ borderColor: accentColor }}></div>
            ...
            <div className="mt-6 flex justify-between items-end border-t border-white/10 pt-4">
              ...
              <div className="text-xs font-bold uppercase tracking-widest animate-pulse" style={{ color: accentColor }}>
                {isAnniversary ? 'Anniversary Active' : 'Awaiting Authorization'}
              </div>
            </div>
          </div>
        </div>
